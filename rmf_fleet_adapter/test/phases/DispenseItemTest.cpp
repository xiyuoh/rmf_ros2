...
SCENARIO("dispense item phase", "[phases]")
{
  std::cout << "ENTER" << std::endl;
  std::shared_ptr<rclcpp::Node> node;
  {
    MockAdapterFixture fixture;
    std::mutex m;
    std::condition_variable received_requests_cv;
    std::list<DispenserRequest> received_requests;
    auto rcl_subscription =
      fixture.data->adapter->node()->create_subscription<DispenserRequest>(
    ...
    const auto info = fixture.add_robot();
    const auto& context = info.context;

    auto dispenser_request_pub =
      fixture.data->adapter->node()->create_publisher<DispenserRequest>(
    ...
    WHEN("it is started")
    {
      ...
      {
        auto result_pub = fixture.data->ros_node->create_publisher<DispenserResult>(
          DispenserResultTopicName, 10);
        auto state_pub = fixture.data->ros_node->create_publisher<DispenserState>(
          DispenserStateTopicName, 10);
        auto timer =
          fixture.data->ros_node->create_wall_timer(std::chrono::milliseconds(100), [&]()
            {
              std::unique_lock<std::mutex> lk(m);
              DispenserResult result;
              result.request_guid = request_guid;
              result.status = DispenserResult::SUCCESS;
              result.time = fixture.data->ros_node->now();
              result_pub->publish(result);

              DispenserState state;
              state.guid = request_guid;
              state.mode = DispenserState::IDLE;
              state.time = fixture.data->ros_node->now();
