  ...
  const BidNotice::SharedPtr msg)
{
  ...
  for (std::size_t i = 0; i < assignments.size(); ++i)
  {
    ...
    for (const auto& a : assignments[i])
    {
      ...
      const double request_seconds = a.request()->earliest_start_time().time_since_epoch().count();
      const double start_seconds = a.deployment_time().time_since_epoch().count();
